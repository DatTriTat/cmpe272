export default function StageOne({ careers, selected, setSelected, onNext }) {
    return (
      <div className="mt-6 bg-white p-6 rounded-xl shadow-md w-full max-w-2xl">
        <h2 className="text-2xl font-bold mb-4">Select a Suggested Career Path</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          {careers.map((career, idx) => (
            <div
              key={idx}
              onClick={() => setSelected(career.title)}
              className={`cursor-pointer border p-4 rounded-lg shadow-sm transition-all hover:shadow-md ${selected === career.title ? 'border-blue-600 bg-blue-50' : 'border-gray-300 bg-white'}`}
            >
        ...
        {selected && (
          <>
            ...
            <p className="text-gray-600 mb-4">{careers.find(c => c.title === selected)?.description}</p>
            <h3 className="font-semibold text-gray-800 mb-1">Required Skills</h3>
            <ul className="list-disc list-inside text-gray-600 mb-6">
              {careers.find(c => c.title === selected)?.skills.map((skill, idx) => (
                <li key={idx}>{skill}</li>
              ))}
            </ul>
            <button
